...
/*   Updated: 2019/07/28 20:37:40 by mobouzar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		d_manage_mines(t_format f, char *str)
{
	int	ret;
	int	i;
...
char	*d_manage_plus(t_format f, char *str)
{
	char	*dst;
	int		i;
	int		j;
...
int		d_manage_space(t_format f, char *str)
{
	int		ret;
	int		i;
...
int		d_manage_options(t_format f, char *str)
{
	...
	!ft_strchr(f.flag, ' '))
		ret += d_manage_mines(f, str);
	else if (ft_strchr(f.flag, '+'))
	{
		str = d_manage_plus(f, str);
		ret += ft_str_nbr(str);
	//	free(str);
	}
	else if (ft_strchr(f.flag, '+') && ft_strchr(f.flag, '-'))
...
char	*d_manage_zero(t_format f, char *str)
{
	char	*dst;
	int		i;
...
char	*d_manage_precision(t_format f, char *str)
{
	char	*dst;
	int		i;
...
int		d_manage_flags(t_format f, char *str)
{
	char	*dst;
	int		len;
...
int		ft_print_d(va_list list, t_format *f)
{
	...
	str = ft_itoa(nbr);
	if (f->precision > 0)
		str = d_manage_precision(*f, str);
	len += d_manage_flags(*f, str);
	//free(str);
	return (len);
}
